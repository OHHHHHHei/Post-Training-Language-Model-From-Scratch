...
import sys
from post_training.modal_utils import app, quote_command, submit_commands

def build_run_commands(args):
    # Suppose args.seeds = '0,1,2,3'
    return [
        [sys.executable, "-u", "scripts/training/train_grpo.py", "--seed", seed]
...
if SUNET_ID == "TODO":
    raise ValueError("Please set SUNET_ID in post_training/modal_utils.py before running Modal jobs.")
...
image = (
    ...
    .uv_sync(extras=["gpu"])
    .workdir(REMOTE_ROOT)
    .add_local_dir("post_training", f"{REMOTE_ROOT}/post_training")
